...
func (t *TrudyPipe) WriteToClient(buffer []byte) (n int, err error) {
	...
	err = t.clientConn.SetWriteDeadline(time.Now().Add(5 * time.Second))
	//if err != nil {
	//	return
	//}
...
func (t *TrudyPipe) ReadFromServer(buffer []byte) (n int, err error) {
	//t.serverConn.SetReadDeadline(time.Now().Add(5 * time.Second))
...
func (t *TrudyPipe) WriteToServer(buffer []byte) (n int, err error) {
	//err = t.serverConn.SetWriteDeadline(time.Now().Add(5 * time.Second))
	if err != nil {
		return
	}
